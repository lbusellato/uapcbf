...
4. Open a shell, activate your target python3 envoironment, then cd inside this directory (or ws, see above), and build the package using ros2 
    `colcon build --packages-select leap_forecasting`
5. Source the workspace 
    `source install/setup.bash`


## Usage
Run the node `ros2 run leap_forecasting leap_forecasting`
or
`ros2 launch leap_forecasting nn_forecasting.launch.py debug:=true`


# PER DUMMY
leggi setup.py e guarda entry_points e console_scripts
poi fai 
```ros2 run leap_forecasting <nome_script>```
